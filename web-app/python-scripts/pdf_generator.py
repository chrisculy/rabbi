...
Usage: python pdf_generator.py <markdown_file> <title> <date> <output_path>
"""
import sys
import os
...
def export_to_pdf_file(guide_markdown, video_title, video_publish_date, output_filename):
    ...
    # Convert all assets to base64 data URLs to avoid file access issues
    def file_to_base64_url(filepath, mime_type):
    ...
    logo_url = file_to_base64_url(logo_path, 'image/png')
...
if __name__ == "__main__":
    if len(sys.argv) < 5:
        print("Usage: python pdf_generator.py <markdown_file> <title> <date> <output>")
        sys.exit(1)
